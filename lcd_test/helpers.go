...
import (
	...
	gapp "gitlab.bianjie.ai/iridas/iridas/app"
)
...
func InitializeLCD(nValidators int, initAddrs []sdk.AccAddress, minting bool, portExt ...string) (
	...
	db := dbm.NewMemDB()
	app := gapp.NewIrisApp(logger, db, nil, true, 0, baseapp.SetPruning(store.PruneNothing))
	cdc = gapp.MakeCodec()
...
func startTM(
	...
	privVal tmtypes.PrivValidator, app *gapp.IrisApp,
) (*nm.Node, error) {
