...
func initRootCmd(rootCmd *cobra.Command, encodingConfig params.EncodingConfig) {
	...
	server.AddCommands(rootCmd, app.DefaultNodeHome, newApp, createIritaappAndExport, addModuleInitFlags)
...
// createIritaappAndExport creates a new IritaApp (optionally at a given height)
// and exports state.
func createIritaappAndExport(
	logger log.Logger, db dbm.DB, traceStore io.Writer, height int64, forZeroHeight bool, jailAllowedAddrs []string,
	appOpts servertypes.AppOptions,
) (servertypes.ExportedApp, error) {
	encCfg := app.MakeEncodingConfig() // Ideally, we would reuse the one created by NewRootCmd.
	encCfg.Marshaler = codec.NewProtoCodec(encCfg.InterfaceRegistry)
	var iritaApp *app.IritaApp
	if height != -1 {
		iritaApp = app.NewIritaApp(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), encCfg, appOpts)
		if err := iritaApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		iritaApp = app.NewIritaApp(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), encCfg, appOpts)
	}
